...
func NewCollector(db database.Connection, nodes *runtime.Nodes, sites []string, ifaces []runtime.InterfaceConfig) *Collector {
...
func (coll *Collector) listenUDP(iface runtime.InterfaceConfig) {
	if _, found := coll.ifaceToConn[iface.Name]; found {
		log.Panicf("can not listen twice on %s", iface.Name)
	}
	...
	if iface.IP != "" {
		...
		addr, err = getLinkLocalAddr(iface.Name)
		if err != nil {
			log.Panic(err)
		}
	}
	...
	coll.ifaceToConn[iface.Name] = conn
